...
import { getDashboardStats, getAllListings } from '../services/dashboard.services.js'
...
export const handleGetListings = async (req: Request, res: Response) => {
    try {
        const id = req.user.id // Admin's or Agent's Id
        const role = req.user.role // User's role i.e either admin or agent

        const data = await getAllListings(id, role as 'admin' | 'agent')
        ...
        error:
            error instanceof Error
            ? error.message
            : 'Failed to get Admin/Agent Listings'
        })
    }
}